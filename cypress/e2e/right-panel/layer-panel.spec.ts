const layerListClassPrefix =
  "src-web-app-views-beambox-Right-Panels-LayerPanel-LayerList-module__";
const addLayerBtnPrefix =
  "src-web-app-components-beambox-right-panel-AddLayerButton-module__btn";
...
describe("manipulate layers", () => {
  ...
  it("there is only one layer by default", () => {
    cy.get(`div[class*="${layerListClassPrefix}item"]`).should(
      "have.length",
      1
    );
    cy.get(`div[class*="${layerListClassPrefix}current"]`).should(
      "have.length",
      1
    );
    cy.get(
    ...
    cy.get("div#laser-panel div.layername").should(
      "have.text",
      "Parameter Settings (Layer 1)"
    );
  ...
  it("rename the new layer", () => {
    ...
    cy.get("div#laser-panel div.layername").should(
      "have.text",
      "Parameter Settings (Hello Flux)"
    );
  });

  it("delete the layer", () => {
    cy.get(`div[class*="${layerListClassPrefix}item"]`).trigger("mousedown", {
      button: 2,
    });
  ...
  it("drag the layer ", () => {
    cy.get(`button[class*="${addLayerBtnPrefix}"]`).click({ force: true });
    cy.get(
      `div[class*="${layerListClassPrefix}item"][data-testid="Layer 2"]`
    ).dragTo('[data-index="0"]');
    cy.get(`div[class*="${layerListClassPrefix}item"]`)
      .eq(1)
      .should("have.text", "Layer 2");
  ...
  it("switch the layer and check the parameter ", () => {
    cy.get("#laser-config-dropdown").click({ force: true });
