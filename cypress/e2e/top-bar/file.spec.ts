...
describe('manipulate file', () => {
  ...
  it('save file', () => {
    ...
    cy.readFile(cypressDownloadBeamPath).then((info) => {
      cy.wrap(md5(info)).should('satisfy', (info) => {
        return info === 'f2ac03b0bb118666656ca2a164ab3609' || info === '05306ee6546f831bc1e90dd8891364ce'
      });
    });
  });
  ...
  it('save as file', () => {
    ...
    cy.readFile(cypressDownloadNewBeamPath).then((info) => {
      cy.wrap(md5(info)).should('satisfy', (info) => {
        return info === 'fe7e2f088f387c0bf225db80a75c85c4' || info === '6f55b26fe235d9ee4edb0ed1343006de'
      });
    });
  });
  ...
  it('export bvg file ', () => {
    ...
    cy.readFile(cypressDownloadBvgPath).then((info) => {
      cy.wrap(md5(info)).should('satisfy', (info) => {
        return info === '857db785d4256f4871918d758c54108f' || info === 'bc940d355184270cf39bc7ca9c6d6df2'
      });
    });
  });
  ...
  it('export svg file ', () => {
    ...
    cy.readFile(cypressDownloadSvgPath).then((info) => {
      cy.wrap(md5(info)).should('satisfy', (info) => {
        return info === 'b919bee615ef8dfa6f394c79e5537d51' || info === '1607e404a7da571a5e6388949fb73cf2'
      });
    });
  });
