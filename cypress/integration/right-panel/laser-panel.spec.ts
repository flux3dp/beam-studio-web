describe('manipulate laser panel', () => {
  ...
  it('import parameter file', () => {
    const downloadPath = Cypress.env('downloadPath');
    cy.get('[title="Import"] > img').click({ force: true });
    cy.fixture('download.json').then(fileContent => {
    ...
    // cy.fixture('download.json').then(function (data) {
    //   cy.get(data);
    // });

    // cy.fixture('download.json').then((todo) => {
    ...
    cy.contains('Hi Flux').should('exist');
  });
