...
Cypress.Commands.add("loginAndLandingEditor", (opts = {}) => {
  ...
  cy.get(".ant-modal-content")
    .should("exist")
    .and("have.text", "Successfully logged in.OK");
...
Cypress.Commands.add(
  ...
  (subject, targetEl) => {
    ...
    cy.get(
      'div[class*="src-web-app-views-beambox-Right-Panels-LayerPanel-LayerList-module__drag-bar--"]'
    ).should("be.visible");
...
Cypress.Commands.add("connectMachine", (machineName: string) => {
  ...
  cy.get(".ant-modal-footer .ant-btn-primary", { timeout: 150000 })
    .contains("Yes")
    .click();
...
Cypress.Commands.add("go2Preference", (handleSave = false) => {
  ...
  cy.get(
    "li.rc-menu__submenu:nth-child(1) li.rc-menu__item:last-child"
  ).click();
...
Cypress.Commands.add("changeWorkarea", (workarea: string, save = true) => {
  ...
  cy.contains(".ant-form-item", "Working Area")
    .find(".ant-select")
    .as("select");
